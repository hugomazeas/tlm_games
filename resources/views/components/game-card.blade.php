...
<div class="bg-white/5 border border-white/10 rounded-xl p-6 hover:bg-white/10 transition group">
    ...
    <h3 class="text-lg font-bold mb-1" style="color: {{ $game->color }}">{{ $game->name }}</h3>
    <p class="text-sm text-white/50 mb-4">{{ $game->description }}</p>
    <div class="flex items-center gap-3">
        ...
        @endif
        <a href="{{ url('/leaderboards/' . $game->slug) }}" class="text-white/60 hover:text-white transition text-xs font-medium ml-auto">
            Leaderboard &rarr;
        </a>
    </div>
</div>
