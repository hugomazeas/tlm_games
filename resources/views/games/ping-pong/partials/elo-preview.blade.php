        ...
        <template x-for="pid in previewPlayerIdsForSide('{{ $side }}')" :key="'preview-{{ $side }}-' + pid">
            <div class="flex flex-col gap-1.5 mb-2 last:mb-0">
                <template x-if="mode === '2v2'">
                    <div class="pph-mono text-[14px] tracking-[0.22em] uppercase text-[#f5ecd6]/55 font-bold" x-text="playerNameById(pid)"></div>
                </template>
                <div class="flex justify-between items-baseline gap-2.5">
                    <span class="pph-mono text-[14px] tracking-[0.22em] uppercase text-[#f5ecd6]/60 font-semibold">If wins</span>
                    <span class="text-right">
                        <span class="pph-mono font-bold text-[24px] tracking-tight"
                        ...
                        <template x-if="(eloPreviewFor(pid, true)?.streak ?? 0) > 0 || (eloPreviewFor(pid, true)?.breaker ?? 0) > 0">
                            <span class="pph-mono text-[14px] text-[#f5ecd6]/45 ml-1.5">
                ...
                <div class="flex justify-between items-baseline gap-2.5">
                    <span class="pph-mono text-[14px] tracking-[0.22em] uppercase text-[#f5ecd6]/60 font-semibold">If loses</span>
                    <span class="pph-mono font-bold text-[24px] tracking-tight"
