...
<div class="flex flex-col gap-px">
    {{-- Header (md+) --}}
    <div class="hidden md:grid grid-cols-[50px_1fr_88px_110px_130px_72px_auto] items-center gap-3 px-3 pt-1 pb-2 mb-1 pph-mono text-[14px] tracking-[0.24em] uppercase text-[#f5ecd6]/45 border-b border-[#f5ecd6]/15 sticky top-0 bg-[#0a0f24] z-10">
        <span>Rank</span>
        <span>Player</span>
        <span>ELO</span>
    ...
    <template x-for="(entry, i) in {{ $entriesExpr }}" :key="entry.player_id">
        <a :href="'/games/ping-pong/players/' + entry.player_id"
           ...
           :class="{
            ...
            {{-- Rank --}}
            <span class="pph-display text-[28px] md:text-[33px] leading-none tracking-wide"
            ...
            {{-- Name --}}
            <span class="font-bold text-[19px] md:text-base text-[#f5ecd6] truncate" x-text="entry.player_name"></span>

            {{-- ELO --}}
            <span class="pph-mono font-bold text-[16px] md:text-[19px] tracking-tight px-2.5 py-1 rounded-md justify-self-end md:justify-self-start"
            ...
            <span class="hidden md:flex flex-col items-center leading-tight gap-px">
                <span class="pph-mono text-[16px] font-bold whitespace-nowrap">
                ...
                <span class="pph-mono text-[15px] text-[#f5ecd6]/45 tracking-wide" x-text="entry.win_rate + '%'"></span>
            </span>
            ...
            <span class="hidden md:block text-center">
                <template x-if="entry.last_10 && entry.last_10.length > 0">
                    <span>
                        <span class="block pph-mono text-[15px] text-[#f5ecd6]/80 mb-1 font-medium" x-text="entry.last_10.filter(r => r === 'W').length + '-' + entry.last_10.filter(r => r === 'L').length"></span>
            ...
            <span class="text-center">
                <template x-if="entry.win_streak > 0">
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full pph-mono font-bold text-[15px] tracking-wide bg-[#9be7c4]/[0.16] text-[#9be7c4]"><span>W</span><span x-text="entry.win_streak"></span></span>
                </template>
                <template x-if="entry.win_streak === 0 && entry.losing_streak > 0">
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full pph-mono font-bold text-[15px] tracking-wide bg-[#ff5a4a]/[0.16] text-[#ff5a4a]"><span>L</span><span x-text="entry.losing_streak"></span></span>
                </template>
            ...
            <template x-if="mode !== '2v2'">
                <span class="hidden md:flex flex-col items-center leading-tight gap-px">
                    <span class="pph-mono text-[16px] font-bold text-[#f5ecd6]" x-text="entry.office_7d + '%'"></span>
                    <span class="pph-mono text-[14px] text-[#f5ecd6]/45 tracking-wide" x-text="entry.office_30d + '% · 30d'"></span>
                </span>
            </template>
        </a>
    </template>
