...
@section('content')
    <div class="mb-8">
        <h1 class="text-3xl font-extrabold tracking-tight mb-2">Leaderboards</h1>
            ...
            <a href="{{ url('/leaderboards/' . $game->slug) }}"
               class="bg-white/5 border border-white/10 rounded-xl p-6 hover:bg-white/10 transition group block">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <div>
                        <h3 class="font-bold" style="color: {{ $game->color }}">{{ $game->name }}</h3>
                        @if($game->is_active)
                ...
                <p class="text-sm text-white/50">{{ $game->description }}</p>
            </a>
        @endforeach
    </div>
@endsection
