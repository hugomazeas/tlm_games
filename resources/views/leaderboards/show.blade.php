            ...
            @foreach($modes as $mode)
                <a href="{{ url('/leaderboards/' . $gameType->slug . '/' . $mode->slug) }}"
                   class="bg-white/5 border border-white/10 rounded-xl p-4 sm:p-6 hover:bg-white/10 transition group block">
                    <h3 class="font-bold text-base sm:text-lg mb-1.5 sm:mb-2" style="color: {{ $gameType->color }}">{{ $mode->name }}</h3>
                    @if($mode->description)
                        <p class="text-xs sm:text-sm text-white/50">{{ $mode->description }}</p>
                    @endif
                </a>
            @endforeach
        </div>
